...
import { addHighlightMarkOpacityRules, addHoverMarkOpacityRules } from '@specBuilder/chartTooltip/chartTooltipUtils';
 ...
 * @param props
 * @param opacity inital opacity of the mark
 * @param hoverOpacity opacity of the mark when hovered
...
export const getMarkOpacity = (
	...
	const rules: ({ test?: string } & NumericValueRef)[] = [
		{ value: opacity !== undefined ? opacity : DEFAULT_OPACITY_RULE.value },
	];
	...
	// if mark is hoverable add hover opacity rules
	if (hoverOpacity !== undefined) {
		addHoverMarkOpacityRules(rules, props, hoverOpacity);
	} else {
		// if a bar is hovered/selected, all other bars should have reduced opacity
		addHighlightMarkOpacityRules(rules, props);
	}

	// if provided a hover opacity make the opacity of the mark the inital opacity
	if (hasPopover(children)) {
		return [
			{
				test: `!isValid(${SELECTED_GROUP}) && ${SELECTED_ITEM} && ${SELECTED_ITEM} !== datum.${idKey}`,
				value: hoverOpacity ? opacity : 1 / HIGHLIGHT_CONTRAST_RATIO,
			},
